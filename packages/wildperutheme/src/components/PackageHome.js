...
import {ourGreatPackages} from '../Root';

const PackagesHome = ({state, actions, libraries}) => {
    ...
    if(data.isReady) {
        data.items.map( ({id}) => {
            ...
            if(singleTour.tags.length>0 && singleTour.tags[0] === 8 || singleTour.tags[1] === 8) {
...
export default connect(PackagesHome);



/**HOME PACKAGES TOURS STYLES */
...
`
/**END HOME PACKAGES TOURS STYLES */
