...
const HikingTours = ({state, actions}) => {
    ...
    if(data.isReady) {
        data.items.map( ({id}) => {
            ...
            if(singleTour.tags.length>0 && singleTour.tags[0] === 10 || singleTour.tags[1] === 10) {
                tours.push(singleTour);
            }
        })
    }

    const pageMachu = state.source.page[524];
...
export default connect(HikingTours);
