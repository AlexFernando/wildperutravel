...
const JungleTours = ({state, actions}) => {
    ...
    if(data.isReady) {
        data.items.map( ({id}) => {
            ...
            if(singleTour.tags.length>0 && singleTour.tags[0] === 11 || singleTour.tags[1] === 11) {
                tours.push(singleTour);
            }
        })
    }

    const pageMachu = state.source.page[526];
...
export default connect(JungleTours);
